...
_logger_process: Optional[subprocess.Popen] = None
_experiment_timer: Optional[asyncio.Task] = None
...
async def stop_experiment_logic():
    global _logger_process, _experiment_timer
    if _logger_process:
        print("Stopping experiment logger...")
        _logger_process.terminate()
        try:
            _logger_process.wait(timeout=2)
        except subprocess.TimeoutExpired:
            _logger_process.kill()
        _logger_process = None
        
    if _experiment_timer:
        _experiment_timer.cancel()
        _experiment_timer = None
        
    wslog("info", "Experiment stopped.")
    await emit({"type": "EXPERIMENT_STOPPED"})

async def handle_start_experiment(payload: dict):
    global _logger_process, _experiment_timer
    
    if _logger_process:
        wslog("warn", "Experiment already running!")
        return

    # Generate filename
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"jetson_resources_{ts}.csv"
    
    try:
        cmd = []
        if IS_REMOTE_LOGGER:
            print(f"Spawning REMOTE logger: {filename}")
            # Note: We pass the filename, but script expects it as first arg.
            # SSH execution: ssh host "python3 script.py filename"
            # We need to construct the remote command string carefully or pass as list
            remote_py_cmd = f"python3 {REMOTE_SCRIPT_PATH} {filename}"
            cmd = REMOTE_SSH_CMD + [remote_py_cmd]
        else:
            print(f"Spawning LOCAL logger: {filename}")
            if sys.platform == "win32":
                python_exe = sys.executable
            else:
                python_exe = "python3"
            cmd = [python_exe, "jetson_logger.py", filename]

        _logger_process = subprocess.Popen(cmd)
        
        wslog("info", f"Experiment STARTED. logging to {filename}")
        await emit({"type": "EXPERIMENT_STARTED", "payload": {"filename": filename}})
        
        # Schedule auto-stop in 600s
        async def auto_stop():
            await asyncio.sleep(EXPERIMENT_DURATION)
            await stop_experiment_logic()
            
        _experiment_timer = asyncio.create_task(auto_stop())
        
    except Exception as e:
        wslog("error", f"Failed to start logger: {e}")
        print("Start experiment error:", e)

async def handle_stop_experiment(payload: dict):
    await stop_experiment_logic()
...
HANDLERS = {
    ...
    "STOP_EXPERIMENT": handle_stop_experiment,
}

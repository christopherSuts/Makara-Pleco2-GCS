...
function validatePerimeterJSON(obj) {
    // Minimal validation: what your saver produces
    if (!obj || obj.type !== "asv-perimeter-track") return "Invalid type.";
    if (!Array.isArray(obj.points) || obj.points.length < 2) return "Points array missing or too short.";
    const bad = obj.points.find(p => typeof p?.lat !== "number" || typeof p?.lng !== "number");
    if (bad) return "Each point must have numeric lat/lng.";
    return null;
}

export default function LoadPerimeterModal({ open, onClose, onImport }) {
    ...
    const handleFile = async (file) => {
        ...
        try {
            ...
            const err = validatePerimeterJSON(obj);
            if (err) { setError(err); return; }
            onImport(obj); // delegate to parent (will set recordedTrack etc)
        } catch (e) {
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center">
            <div className="w-full max-w-xl rounded-2xl bg-white text-black shadow-xl">
                ...
                <div className="flex items-center justify-between px-5 py-3 border-b border-black/10">
                    <h3 className="text-lg font-semibold">Load Saved Perimeter</h3>
                ...
                <div className="px-5 py-4">
                    {tab === "offline" && (
                        <div className="space-y-3">
                            <p className="text-sm text-black/70">
                                Select the <code>.json</code> file you downloaded earlier (created by “Save JSON”).
                            </p>
                            <input
                                type="file"
                    ...
                    {tab === "online" && (
                        <div className="space-y-3">
                            <p className="text-sm text-black/70">
                                Enter a URL to a perimeter JSON (your API, S3/MinIO signed URL, etc.).
                            </p>
                            <input
                                type="url"
                                value={onlineUrl}
                                onChange={(e) => setOnlineUrl(e.target.value)}
                                placeholder="https://example.com/perimeters/123.json"
                            ...
                            <p className="text-[12px] text-black/50">
                                Note: The URL must allow browser access (CORS). For private stores, use your Next.js API as a proxy.
                            </p>
                        </div>
                    )}
